...
public:
	class Node 
	{ // singly-linked nodes (basic containers of a linked list)  
	public:
...
{
	if (isEmpty()) addHead(item);       //adds head if empty
	else
	{
        Node * someNode = new Node(item); //creating a new node
        m_tail->next = someNode;        //assigning new node to be next node of current tail
        m_tail = someNode;              //assigning the new node to become the tail
        m_tail->next = m_head;          //assigning the new tail node's next pointer to the head
		m_size++;
	}
}
